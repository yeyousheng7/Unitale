...
<template>
  <div :class="['script-line-card script-dialogue-card group cursor-pointer border',
    ...
    <div class="dialogue-order-rail">
      <button @click.stop="moveLineUp(index)" class="dialogue-order-button" title="上移" aria-label="上移">
        <svg class="ui-icon" aria-hidden="true"><use href="../../../assets/icons/ui-icons.svg#chevron-up"></use></svg>
      </button>
      <span>{{ index + 1 }}</span>
      <button @click.stop="moveLineDown(index)" class="dialogue-order-button" title="下移" aria-label="下移">
    ...
    <div class="dialogue-main">
      <div class="dialogue-card-actions">
        ...
        <button @click.stop="generateLineAudio(line)" class="dialogue-icon-button"
          ...
          :aria-label="line.isGenerating ? '停止生成音频' : '生成音频'">
      ...
      <textarea v-model="line.text"
        ...
        class="line-textarea dialogue-textarea" aria-label="台词内容"
      ...
      <div class="dialogue-parameters">
        ...
        <div class="dialogue-sfx">
          <div v-for="(sfx, sIdx) in line.sfx" :key="sIdx" class="dialogue-sfx-chip">
            <select v-model="sfx.name" aria-label="音效名称">
            ...
            <input type="number" v-model="sfx.position" step="0.1" min="0" max="1"
              title="插入位置 (0.0 - 1.0)" aria-label="音效插入位置">
            <button @click="removeLineSfx(line, sIdx)" class="dialogue-sfx-remove"
              title="移除音效" aria-label="移除音效"><svg class="ui-icon" aria-hidden="true"><use href="../../../assets/icons/ui-icons.svg#x"></use></svg></button>
          </div>
          <button @click="addLineSfx(line)" class="dialogue-add-sfx">
            <svg class="ui-icon" aria-hidden="true"><use href="../../../assets/icons/ui-icons.svg#plus"></use></svg>音效
          </button>
        </div>
      </div>
      ...
      <div v-if="line.audioUrl" class="dialogue-audio-panel">
        <div class="dialogue-range-grid">
          <label class="dialogue-range-field">
            <span><b>台词音量</b><output>{{ Math.round((line.dialogueVolume ?? 1) * 100) }}%</output></span>
            <input type="range" v-model.number="line.dialogueVolume" min="0" max="2" step="0.05" class="accent-blue-600">
          </label>
          <label class="dialogue-range-field">
            <span><b>音效音量</b><output>{{ Math.round((line.sfxVolume || 0.5) * 100) }}%</output></span>
            <input type="range" v-model="line.sfxVolume" min="0" max="2" step="0.05" class="accent-indigo-600">
          </label>
          <label class="dialogue-range-field">
            <span><b>语速</b><output>{{ (line.speed || 1).toFixed(1) }}x</output></span>
            <input type="range" v-model.number="line.speed" min="0.2" max="2" step="0.1" class="accent-green-600">
          </label>
        </div>

        <div class="dialogue-waveform">
          <div class="dialogue-waveform-heading"><span>音频剪辑</span><span>{{ Math.round(((line.trimEnd || 1) - (line.trimStart || 0)) * 100) }}%</span></div>
        ...
        <div class="dialogue-audio-actions">
          <button @click.stop="playLineAudio(line)" title="播放" aria-label="播放音频" class="dialogue-icon-button">
          ...
          <button @click.stop="clearLineAudio(line)" title="清除音频" aria-label="清除音频" class="dialogue-icon-button is-remove">
