        ...
        <div class="script-workspace-switch-row">
          ...
          <details ref="scriptMenuRef" class="script-switcher" @keydown.esc.prevent.stop="closeScriptMenu(true)"
            ...
            <div class="script-switcher-menu">
              <div class="script-switcher-list" role="group" aria-label="脚本列表">
                <div v-for="script in scriptList" :key="script.id"
                  ...
                  <div class="script-switcher-row-main">
                    ...
                    <input v-else v-model="script.name" class="script-switcher-input" aria-label="脚本名称"
                    ...
                    <button type="button" class="script-switcher-more" :aria-label="'管理脚本：' + script.name"
                  ...
                  <div v-if="openActionId === script.id" class="script-switcher-row-actions">
                    <button type="button" @click="renameScript(script.id)">重命名</button>
                    <button type="button" class="is-danger" @click="deleteScript(script.id)">删除脚本</button>
                  </div>
                </div>
              </div>
              <button type="button" class="script-switcher-create" @click="createScript">
                <svg aria-hidden="true"><use href="../../../assets/icons/ui-icons.svg#plus"></use></svg>新建脚本
              </button>
            </div>
          </details>
